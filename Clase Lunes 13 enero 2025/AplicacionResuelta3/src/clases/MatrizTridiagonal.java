 ...
 * Representa productos con tres características clave: nombre, precio y cantidad.
...
public class MatrizTridiagonal {
    ...
    public MatrizTridiagonal(int n) {
        this.n = n;
        matriz = new int[n][3]; // Almacena solo las tres diagonales
    }

    public void setValor(int fila, int diagonal, int valor) {
        if (fila >= 0 && fila < n && diagonal >= -1 && diagonal <= 1) {
            matriz[fila][diagonal + 1] = valor; // -1: izquierda, 0: diagonal, 1: derecha
        } else {
            throw new IllegalArgumentException("Posición fuera de la matriz tridiagonal.");
        }
    }

    public int getValor(int fila, int diagonal) {
        if (fila >= 0 && fila < n && diagonal >= -1 && diagonal <= 1) {
            return matriz[fila][diagonal + 1];
        }
        return 0; // Elementos fuera de la tridiagonal son 0
    }
